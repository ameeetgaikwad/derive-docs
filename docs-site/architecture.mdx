---
title: "Architecture"
description: "The layered design of Hedge: on-chain settlement contracts, an off-chain RFQ auction layer, and a hybrid oracle stack (Pyth+Chainlink spot, signed feeds)."
icon: "layers"
---

Hedge is built as three layers. The bottom layer — the contracts — is where all value lives and all rules are enforced. The layers above it only coordinate signatures and data; they can never move funds outside what a user has signed.
...
flowchart TB
    ...
    subgraph onchain["On-chain (BNB Chain)"]
        ...
        F["Feeds: Pyth+Chainlink spot ·<br/>signed forward · vol surface · rate · stable"]
    end
...
<AccordionGroup>
  ...
  <Accordion title="Oracles — the protocol's marks" icon="radio">
    BTC **spot** comes from a **Pyth adapter with a Chainlink circuit breaker**: Pyth BTC/USD is primary, and a >1% Chainlink deviation zeroes the price's confidence, tripping the margin system's oracle-contingency penalty. Per-expiry forwards, the SVI volatility surface, interest rates, and the USDT/USD stable feed remain on-chain **signed feeds**: updates are signed off-chain by whitelisted signers (with heartbeats and confidence values) and posted on-chain, where anyone can read them. Settlement is still priced off the signed forward print. See [Oracles & feeds](/reference/oracles).
  </Accordion>
</AccordionGroup>
...
| **Oracle feeds service** | Signs and posts the signed feed data (forward/vol/rate/stable), relays Pyth spot updates on demand (`pyth-push`), and runs settlement at expiry. |
...
The settlement contracts are a **fork of audited, open-source protocol contracts** (GPL-3.0 core, AGPL-3.0 matching layer) whose base was audited by Sigma Prime. Hedge pins the forked snapshot, deploys it with its own scripts, and treats the vendored source as read-only; modifications to the AGPL matching layer are published as the license requires. Full licensing detail in [Why BNB Chain & licensing](/why-bnb#licensing-and-provenance).
